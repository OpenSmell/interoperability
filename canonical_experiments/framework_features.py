"""Complete Framework Taxonomy: exhaustive feature extraction for MOX sensor time-series.

Implements all 5 framework categories from the paper:
1. Device-Agnostic (6 per-channel + cross-channel + global)
2. Absolute (4 per-channel)
3. Temporal (4 per-channel)
4. Health (4 per-channel)
5. Hardware (3 per-channel)

Total: 21 per-channel features + cross-channel + global = 21N + N²/2 + 4
"""
...
def compute_channel_absolute(series, r0=None):
    ...
    voltage = float(raw_resistance)  # PLACEHOLDER: requires Vcc and RL; using raw resistance as proxy
...
def compute_channel_health(series, r0_samples=15):
    ...
    drift_rate = float((np.mean(series[-10:]) - r0) / r0) if len(series) >= 10 else 0.0  # PLACEHOLDER: within-recording drift; true drift requires days/weeks

    sensitivity_decay = 0.0  # PLACEHOLDER: requires multiple recordings over months
...
def compute_channel_hardware(series):
    ...
    circuit_response = float(np.mean(series))  # PLACEHOLDER: channel-wise mean — not true circuit characterization

    thermal_profile = float(np.std(series))  # PLACEHOLDER: channel-wise std — not true thermal characterization

    adc_noise = 0.0
    if len(series) >= 10:
        smooth = np.convolve(series, np.ones(5) / 5, mode="valid")
        residuals = series[2:2 + len(smooth)] - smooth
        adc_noise = float(np.std(residuals)) if len(residuals) > 0 else 0.0  # PLACEHOLDER: residual std — not true ADC characterization
