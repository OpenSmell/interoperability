...
- Train RandomForest on **SmellNet (Rig A)** restricted to the three live
  channels shared with Praise James' rig — VOC, Alcohol, LPG (canonical ch 2, 4, 5).
- Feature set: the **canonical 91-dim extractor subset** for those channels
  (28 per-channel dimensions × 3 + 3 selectivity ratios + 4 globals).
- Test set: Praise James' real V2 recordings (garlic ×2, cinnamon ×1, banana ×1).
  lime is Praise James-only and used as an out-of-vocabulary probe.
...
- Alignment: per-live-channel single-point M = median(SmellNet rel-amp) /
  median(Praise James' rel-amp), fit on reference substances present in **both**
...
| RAW (no alignment), Praise James' rig | **8.3%** (1/12) | cinnamon 0/3, garlic 0/6, banana 1/3 — all collapse to banana |
...
The banana recording's VOC and LPG channels **clip to exactly 0.0** (ADC rail).
...
With the banana file excluded (test = garlic + cinnamon only):
...
1. **The full canonical model is dominated by device-bound features.** Top-14
   ...
   scales that are meaningless on another rig → Praise James' samples land in
...
3. **Magnitude-only model (15 dims) also fails (0.0%).** Praise James' rig's
   ...
   recordings (e.g., banana LPG rel-amp = 1.0 vs SmellNet banana = 0.11),
...
- The failure is **not** evidence that M is mathematically wrong; it is
  ...
  features, and (b) Praise James' recordings carry exposure-intensity variance +
...
- sr mismatch: Praise James' rig ≈ 2.1 Hz, SmellNet assumed 10 Hz → kinetic features
